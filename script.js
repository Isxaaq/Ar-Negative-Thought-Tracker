...
function addThought() {
    ...
    if (!date || newThought === "") return alert("يرجى اختيار التاريخ وإدخال فكرة!");
...
function calculateRatio() {
    let date = document.getElementById("date").value;
    if (!date || !thoughtsData[date]) return alert("لم يتم العثور على أفكار لهذا التاريخ!");
    ...
    document.getElementById("result").innerText = `نسبة تحقق الأفكار السلبية: ${percentage}%`;
...
function updateChart() {
    ...
    window.myChart = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: "نسبة تحقق الأفكار السلبية (%)",
                data: data,
                borderColor: "blue",
                fill: false
            }]
